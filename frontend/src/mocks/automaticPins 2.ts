import Halloween24 from "@/assets/pins/Halloween_event24.svg";
import ChristmasEvent24 from "@/assets/pins/Christmas_event24.svg";
import SummerEvent24 from "@/assets/pins/Summer_event24.svg";
import Test_pin from "@/assets/pins/Test_pin.svg";

export const automaticPins = [
  {
    id: "pizza_day",
    imagePin: Test_pin,
    pinTitle: "Pizza Day 0409",
    pinDescription: "Test Event",
    eventDate: "2024-09-04",
    autoAssigned: true,
  },
  {
    id: "summer_event_2024",
    imagePin: SummerEvent24,
    pinTitle: "Summer Event 2024",
    pinDescription:
      "The 'Event Summer 2024' pin is awarded for attending the Event...",
    eventDate: "2024-09-20",
    autoAssigned: true,
  },
  {
    id: "halloween_2024",
    imagePin: Halloween24,
    pinTitle: "Halloween Event 2024",
    pinDescription:
      "The 'Event Halloween 2024' pin is awarded for attending the Event...",
    eventDate: "2024-10-31",
    autoAssigned: true,
  },
  {
    id: "christmas_2024",
    imagePin: ChristmasEvent24,
    pinTitle: "Chrismast Event 2024 ",
    pinDescription:
      "The 'Event Christmas 2024' pin is awarded for attending the Event organized by Rindus in November 2024. Congratulations!",
    eventDate: "2024-11-29",
    autoAssigned: true,
  },
  {},
];
