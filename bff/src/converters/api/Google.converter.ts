...
import {
  MinimalEvent as MinimalEvent,
  DetailedEvent as DetailedEvent,
...
export class MinimalEventConverter
  implements Converter<calendar_v3.Schema$Event, MinimalEvent>
{
  convert(event: calendar_v3.Schema$Event): MinimalEvent {
    if (
      !event ||
      !event?.id ||
      !event?.summary ||
...
export class DetailedEventConverter
  implements Converter<calendar_v3.Schema$Event, DetailedEvent>
{
  convert(event: calendar_v3.Schema$Event): DetailedEvent {
    if (
      !event ||
      !event?.id ||
      !event?.summary ||
